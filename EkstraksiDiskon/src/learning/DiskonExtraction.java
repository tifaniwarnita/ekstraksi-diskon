...
public class DiskonExtraction {
    ...
    private static final String REGEX_TANGGAL = "((?:[\\d]{1,2}(?:(?:st|nd|rd|th|th) (?:of)?)?[-/\\s]*(?:jan(?:uar[i|y])?|feb(?:ruar[i|y])?|mar(?:et|ch)?|apr(?:il)?|mei|may|jun[i|e]?|jul[i|y]?|aug(?:ust)?|agustus|sept?(?:ember)?|o[c|k]t(?:ober)?|nov(?:ember)?|de[c|s](?:ember)?)?\\s*(?:(?:'?\\d{2}\\s)|\\d{4})?\\s*(?:-|s\\.?d\\.?|sampai|sampai dengan)\\s*)?[\\d]{1,2}(?:(?:st|nd|rd|th|th) (?:of)?)?[-/\\s]*(?:jan(?:uar[i|y])?|feb(?:ruar[i|y])?|mar(?:et|ch)?|apr(?:il)?|mei|may|jun[i|e]?|jul[i|y]?|aug(?:ust)?|agustus|sept?(?:ember)?|o[c|k]t(?:ober)?|nov(?:ember)?|de[c|s](?:ember)?)\\s*(?:(?:'?\\d{2}\\s)|\\d{4})?)|(\\d{1,2}[-/]\\d{1,2}[-/]\\d{4})";
